...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create add_do-1d04
  fly apps create add_do-1d04-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app add_do-1d04
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app add_do-1d04-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'add_do-1d04-staging' does not match app name in config file 'add_do-1d04'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name add_do-1d04-db
  fly postgres attach --app add_do-1d04 add_do-1d04-db

  fly postgres create --name add_do-1d04-staging-db
  fly postgres attach --app add_do-1d04-staging add_do-1d04-staging-db
  ```
